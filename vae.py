...
class DatasetCustom(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label[label_idx] = 1.0
        return torch.FloatTensor(np_x), torch.FloatTensor(np_y), torch.FloatTensor(label)
...
def main():
    ...
    for stage in ['train', 'test']:
        for metric in ['loss', 'loss_rec', 'loss_kl']:
    ...
    for epoch in range(1, 100):
        for data_loader in [data_loader_train, data_loader_test]:
            ...
            for x, y, _ in data_loader:
                ...
                metrics_epoch[f'{stage}_loss_kl'].append(loss_kl.cpu().item())
        ...
        for i, j in enumerate([4, 5, 6, 16, 17, 18]):
            ...
            plt.subplot(4, 6, j + 6)
            plt.imshow(y_prim.cpu().data.numpy()[i][0].T, cmap=plt.get_cmap('Greys'))
